...
Public Class frmCharging
    ...
    Sub fill()
        ...
        If rs.EOF = False Then
            For lup = 1 To rs.RecordCount
                ...
                viewlst.SubItems.Add(Format(Math.Round(rs("MinAmount").Value, 2), ".00"))
                ...
                viewlst.SubItems.Add(Format(Math.Round(rs("LostCard").Value, 2), ".00"))
                rs.MoveNext()
            Next
        End If
    End Sub
    ...
    Sub FindS()
        ...
        Dim lup As Short
        rs = New Recordset
        rs = conSqlPOS.Execute("select * from tblCharge where " & cboCateg.Text & " like '" & txtKeyword.Text & "'")
        txtCnt.Text = rs.RecordCount
        If rs.EOF = False Then
            For lup = 1 To rs.RecordCount
                lstList.Refresh()
                ...
                viewlst.SubItems.Add(Format(Math.Round(rs("FlatRate").Value, 2), ".00"))
                viewlst.SubItems.Add(Format(Math.Round(rs("LostCard").Value, 2), ".00"))
                viewlst.SubItems.Add(Format(Math.Round(rs("VAT").Value, 2), "0.00"))
                rs.MoveNext()
            Next
        End If
    End Sub
    ...
    Sub del2()
        ...
        Try
            ...
            saveAudiUpdate(str_SysUser, str_SysPosi, "Charging rule deleted")
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Critical, "Delete")
        End Try
    End Sub
