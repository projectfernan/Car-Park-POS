...
Module AddPic
    ...
    Public Function Get_PicIn(ByRef Acnumber As String) As Bitmap
        Try
            Dim rs As New Recordset
            rs = conSqlPOS.Execute("select PIC from timeindb where CardCode = '" & Acnumber & "' limit 1;")
            If rs.EOF = False Then
                Dim imgByteArray() As Byte
                Try
                    imgByteArray = CType(rs("PIC").Value, Byte())
    ...
    Public Function Get_PicDriver(ByRef Acnumber As String) As Bitmap
        Try
            Dim rs As New Recordset
            rs = conSqlPOS.Execute("select PIC2 from timeindb where CardCode = '" & Acnumber & "' limit 1;")
            If rs.EOF = False Then
                Dim imgByteArray() As Byte
                Try
