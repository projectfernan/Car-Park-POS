...
Module Log_Out_Print
    ...
    Private Sub LOPrint_Print(ByVal sender As System.Object, ByVal e As PrintPageEventArgs) Handles LOPrint.PrintPage
        ...
        rs = conSqlPOS.Execute("select VehicleType,Count(VehicleType)as VehicleCount,sum(total)as VehicleTotalAmt,Total from tblincomereport where Station = '" & strStation & "' and Operator = '" & strOp & "' and TimeOut >= '" & strTime1 & "' and  TimeOut <= '" & strTime2 & "' group by Total,VehicleType order by VehicleType,Total")
        ...
        e.Graphics.DrawString("P" + " " + MakeMoney(NetSale), printFont, Brushes.Black, x + 150, y - 5)

        y += (lineOffset * 2.0)
        e.Graphics.DrawString("Total VAT :", printFont, Brushes.Black, x, y - 5)
        e.Graphics.DrawString("P" + " " + MakeMoney(Vat), printFont, Brushes.Black, x + 150, y - 5)

        y += (lineOffset * 2.0)
        e.Graphics.DrawString("Total VAT Sales :", printFont, Brushes.Black, x, y - 5)
        e.Graphics.DrawString("P" + " " + MakeMoney(VatSale), printFont, Brushes.Black, x + 150, y - 5)
    ...
    Public Function OrFrom(ByVal op As String, ByVal Station As String, ByVal petsa1 As Date, ByVal petsa2 As Date) As String
        ...
        Try
            ...
            Dim rs As New Recordset
            rs = New Recordset
            rs = conSqlPOS.Execute("select Min(ORno) as MinOr from tblincomereport where Station = '" & Station & "' and Operator = '" & op & "' and TimeOut >= '" & dt1 & "' and  TimeOut <= '" & dt2 & "'")
            If rs.EOF = False Then
