...
Module CHARGING_FUNCTION
    ...
    Public Function Guest_FlateRATE(ByVal Timein As Date, ByVal Timeout As Date) As Double
        If conLocal() = False Then Return "0.00"
        Dim rs As New Recordset
        Try
            rs = conSqlLoc.Execute("SELECT * FROM tblguestrate")
            If rs.EOF = False Then
                Dim F As Double = rs("GuestRate").Value
                Dim TDmin As Long = DateDiff(DateInterval.Minute, Timein, Timeout)
                Dim TD As Long = DateDiff(DateInterval.Hour, Timein, Timeout)
    ...
    Public Function viewMonthly() As Double
        If conLocal() = True Then
            Try
                Dim rs As New Recordset
                rs = conSqlLoc.Execute("select * from tblmonthlyrate")
                If rs.EOF = False Then
                    Return rs("Monthlyrate").Value
                Else
                    Return 0
                End If
            Catch ex As Exception
    ...
    Public Function viewInhouse() As Double
        If conLocal() = True Then
            Try
                Dim rs As New Recordset
                rs = conSqlLoc.Execute("select * from tblGuestRate")
                If rs.EOF = False Then
                    Return rs("GuestRate").Value
                Else
                    Return 0
                End If
            Catch ex As Exception
