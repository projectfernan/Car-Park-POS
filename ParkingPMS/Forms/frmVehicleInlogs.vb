...
Public Class frmVehicleInlogs
    ...
    Sub find()
        ...
        If chkTransacted.Checked = True Then
            rs = New Recordset
            rs = conSqlLoc.Execute("SELECT * FROM TimeinDB WHERE Validate = 1 order by Timein ASC") '"select * from timeindb where TimeIn >= '" & TmeInFrm & "' and TimeIn <='" & TmeInTo & "'")
        ...
        Else
            rs = New Recordset
            rs = conSqlLoc.Execute("SELECT * FROM TimeinDB WHERE Plate like '" & txtPlate.Text & "%' or TimeIn >= '" & TmeInFrm & "' AND Timein <= '" & TmeInTo & "' Order by Timein ASC") '"select * from timeindb where TimeIn >= '" & TmeInFrm & "' and TimeIn <='" & TmeInTo & "'")
            If rs.EOF = False Then
                txtcnt.Text = rs.RecordCount
                For lup = 1 To rs.RecordCount
                    lstList.Refresh()
    ...
    Private Sub lstList_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles lstList.SelectedIndexChanged
        ...
        For Each viewlst In lstList.Items
            If viewlst.Selected = True Then
                CarImage.Image = Get_PicDriver(viewlst.SubItems(0).Text)
            End If
        Next
    End Sub
    ...
    Private Sub frmVehicleInlogs_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
