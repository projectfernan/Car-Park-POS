...
Module D8Offline
    ...
    Public Function readCard() As Boolean
        Dim d8Timein As String = d8_Read()

        Try
            tmeIn = Format(CDate(d8Timein), "yyyy-MM-dd HH:mm")
            EntryTime = DateTime.Parse(tmeIn)
            Return True
        Catch ex As Exception
            Try
                tmeIn = Mid(d8Timein, 7, 4) & "-" & Mid(d8Timein, 1, 2) & "-" & Mid(d8Timein, 4, 2) & " " & Mid(d8Timein, 12)
                EntryTime = DateTime.Parse(tmeIn)

                Return True
            Catch ex2 As Exception
                'Save_ErrLogs("[readCard] MainForm", ex2.Message)
                Return False
            End Try
        End Try
