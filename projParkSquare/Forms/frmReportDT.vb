...
Public Class frmReportDT
    ...
    Sub Operators()
        ...
        rs = New Recordset
        rs = conSqlPOS.Execute("Select * from tblUserAcc where Designation = 'Operator'")
        cboOp.Items.Clear()
        While rs.EOF = False
    ...
    Sub VehicleS()
        Dim rs As New Recordset
        rs = New Recordset
        rs = conSqlPOS.Execute("Select * from tblcharge order by VehicleType")
        cboVehicle.Items.Clear()
        While rs.EOF = False
    ...
    Sub reprint(ByVal OrNumber As String)
        ...
        Try
            ...
            If rs.EOF = False Then
                ...
                P_HourLy = Format(Math.Round(Val(rs("Hourly").Value), 2), "0.00")
